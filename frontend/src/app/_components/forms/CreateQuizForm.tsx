...
export default function CreateQuizForm({ refetch }: { refetch: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await toast.promise(axiosInstance.post("/quiz", payload), {
    ...
    <Card className="w-full max-w-3xl mx-auto bg-white shadow-md p-6 rounded-2xl">
      <form onSubmit={handleSubmit}>
        <CardHeader className="flex items-center gap-3 mb-4">
          ...
          <div>
            <CardTitle className="text-lg">{t("AddNewQuiz")}</CardTitle>
